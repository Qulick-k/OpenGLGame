...
OGraphicsEngine::OGraphicsEngine()
{
	...
	wc.lpfnWndProc = DefWindowProc;                  //&WndProc//DefWindowProc//指定視窗處理函數 DefWindowProc負責視窗最小化、最大化、關閉；lpfnWndProc處理發送到窗口的消息，如按鍵輸入、滑鼠事件、繪製請求
	...
	pixelFormatDesc.nSize = sizeof(PIXELFORMATDESCRIPTOR);                               //設置PIXELFORMATDESCRIPTOR結構體的大小
	pixelFormatDesc.nVersion = 1;                                                        //設置結構的版本號為1
	pixelFormatDesc.iPixelType = PFD_TYPE_RGBA | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER;  //設置像素格式類型,包括RGBA顏色模型、支援OpenGL、使用雙緩衝(避免畫面閃爍)
	...
	auto pixelFormat = ChoosePixelFormat(dummyDC, &pixelFormatDesc);                     //(ChoosePixelFormat)選擇最接近的像素格式
	SetPixelFormat(dummyDC, pixelFormat, &pixelFormatDesc);							     //(SetPixelFormat)為設備上下文設置像素格式

	auto dummyContext = wglCreateContext(dummyDC);                                       //創建OpenGL渲染上下文
